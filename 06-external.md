...
## [Data Confidentiality]{.underline}
...
## [Access to the External Server]{.underline}

1. Upon access approval, the User will be able to connect to the external server using one of two possibilities.
  - NoMachine client access ([preferred]{.underline}): see **Appendix 4** for details on installation and use
  - Browser access ([low performance]{.underline}): see **Appendix 5** for further details
...
+-----------------------------------+-----------------------------------+
  ...
  - By default you also have two files (see image above): (1)
    ...
    \"**[sh]{.underline}**\" extension allow you to send commands to
...
## [Using the 'shell' in a Linux Operating System]{.underline}

If you wish to run your
programs in ***[batch]{.underline}*** mode, then you must use the
*'**shell**'* of Linux. You can also use the 'shell' to organize the
files in your working space.

1.  The *'**shell**'*[^5]
    [^[5]{.underline}^](https://translate.googleusercontent.com/#footnote5)
...
height="2.488604549431321in"}
  ...
  - For the interactive graphical mode click on the icons
    "**[xstata14mp.sh]{.underline}**" (Stata 14) or
    "**[xstata15mp.sh]{.underline}**" (Stata 15) located in the
...
## [Additional Statistical Software]{.underline}
...
-   **cp**cp Copy file(s) to a given path

    cp prog1.do /bplimext/projects/I001\_jdoe/results

<!-- -->

-   **mv**mv Move file(s) or rename a file from a given path
...
-   **screen**screen Switch between screen

    screen top

<!-- -->

-   **stata** -- **mpman**man Show the manual page for the given command

    man ls

<!-- -->

-   **du**du -h Check the information of disk usage of files and
    directories.
...
-   **df**df -h Check disk space utilization and show the disk space
...
## [Appendix 2 -- Using the 'vi' file editor]{.underline}
...
## [Appendix 3 -- External server's password requirements]{.underline}
...
## [Appendix 4 -- Download, install and configure NoMachine client]{.underline}
...
> height="3.0316502624671915in"}
>
> **Step 5.7**: Once the entry for bplim.bportugal.pt has been created,
> connect:
>
...
2.  Mac users are not able to install NoMachine, receiving the following
    message
...
3.  NoMachine authentication failure
...
4.  User pressed 'Lock' instead of 'Log out' and the unlock/password
    does not work:
...
5.  "Cannot see the screen in NoMachine" (see image below)
...
[^1]: Dolphin is an intuitive and easy-to-use file manager. You can use it, for example, to browse the directory, to
    ...
    [*[https://userbase.kde.org/Dolphin]{.underline}*](https://translate.google.com/translate?hl=en&prev=_t&sl=pt-BR&tl=en&u=https://userbase.kde.org/Dolphin)
    .
